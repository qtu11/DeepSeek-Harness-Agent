...
async function runGroup(
  ...
  const toolScheduler: ToolRuntimeScheduler =
    (ctx.tools as { [TOOL_RUNTIME_SCHEDULER]?: ToolRuntimeScheduler })[TOOL_RUNTIME_SCHEDULER] ??
    (ctx.tools as { scheduler?: ToolRuntimeScheduler }).scheduler ?? {
      prepare: exec => Promise.resolve({ kind: 'dispatch', exec: exec as ToolRunContext }),
      dispatch: async exec => ({ kind: 'final-result', result: await ctx.tools.execute(exec) }),
      finalize: (_exec, result) => Promise.resolve(result),
      finish: (_exec, result) => result,
    }
  ...
  const commitReady = async (): Promise<void> => {
    while (committed < group.length) {
      ...
      const result = slot.needsPost
        ? await toolScheduler.finalize(slot.exec, slot.result)
        : toolScheduler.finish(slot.exec, slot.result)
  ...
  const startCall = async (index: number): Promise<void> => {
    ...
    started++
    const prepared = await toolScheduler.prepare(call.exec)
    throwSchedulerFailure()
    switch (prepared.kind) {
      case 'dispatch': {
        const promise = toolScheduler.dispatch(prepared.exec).then(
      ...
      default:
        assertNever(prepared as never, 'tool-call scheduler prepare result')
    }
  }
